...
describe('BestScoreBadge', () => {
  it('shows available quick and full best scores independently', async () => {
    saveBestScore(robotFrameworkMetadata, 16, 20)
    saveBestScore(robotFrameworkMetadata, 36, 40)

    const screen = await render(<BestScoreBadge topic={robotFrameworkMetadata} />)
  ...
  it('uses a concise empty state before either mode is completed', async () => {
    ...
    await expect.element(screen.getByText('No attempts yet')).toBeInTheDocument()
  })
})

...
describe('routed application', () => {
  ...
  it('loads the Basic C# topic and its quiz', async () => {
    const user = userEvent.setup()
    renderRoute('/topics/csharp-basics')

    expect(await screen.findByRole('heading', { name: 'Basic C# cheatsheet' })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Microsoft Learn: A tour of C#/ })).toBeInTheDocument()
  ...
  it('loads the JMeter topic and its quiz', async () => {
    const user = userEvent.setup()
    renderRoute('/topics/jmeter')

    expect(
      await screen.findByRole('heading', { name: 'Load Testing with JMeter cheatsheet' }),
    ).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /Apache JMeter: Getting Started/ })).toBeInTheDocument()

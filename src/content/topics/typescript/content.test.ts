...
import typescriptTopic from '.'

describe('TypeScript topic content', () => {
  it('contains ten valid questions of each supported kind', () => {
    expect(typescriptTopic.questions).toHaveLength(40)
    for (const kind of ['multiple-choice', 'text-blank', 'drag-blank', 'sequence'] as const) {
      expect(typescriptTopic.questions.filter((question) => question.kind === kind)).toHaveLength(
        10,
      )
    }
  })
  ...
  it('has unique namespaced IDs and valid answer references', () => {
    const ids = typescriptTopic.questions.map((question) => question.id)
    expect(new Set(ids).size).toBe(ids.length)

    for (const question of typescriptTopic.questions) {
      expect(question.id).toMatch(/^typescript\./)
      expect(question.topicId).toBe(typescriptTopic.id)
  ...
  it('keeps metadata and reference material in sync', () => {
    expect(typescriptTopic.questionCount).toBe(typescriptTopic.questions.length)
    expect(typescriptTopic.subsetQuestionCount).toBeGreaterThan(0)
    expect(typescriptTopic.subsetQuestionCount).toBeLessThan(typescriptTopic.questionCount)
    expect(typescriptTopic.cheatsheet).toHaveLength(6)
    expect(typescriptTopic.lastReviewed).toMatch(/^\d{4}-\d{2}-\d{2}$/)
  })
})

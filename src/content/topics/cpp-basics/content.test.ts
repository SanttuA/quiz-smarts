...
import cppBasicsTopic from '.'

describe('Basic C++ topic content', () => {
  it('contains ten valid questions of each supported kind', () => {
    expect(cppBasicsTopic.questions).toHaveLength(40)
    for (const kind of ['multiple-choice', 'text-blank', 'drag-blank', 'sequence'] as const) {
      expect(cppBasicsTopic.questions.filter((question) => question.kind === kind)).toHaveLength(10)
    }
  })

  it('has unique namespaced IDs and valid answer references', () => {
    const ids = cppBasicsTopic.questions.map((question) => question.id)
    expect(new Set(ids).size).toBe(ids.length)

    for (const question of cppBasicsTopic.questions) {
      expect(question.id).toMatch(/^cpp-basics\./)
      expect(question.topicId).toBe(cppBasicsTopic.id)
  ...
  it('keeps metadata and reference material in sync', () => {
    expect(cppBasicsTopic.questionCount).toBe(cppBasicsTopic.questions.length)
    expect(cppBasicsTopic.subsetQuestionCount).toBeGreaterThan(0)
    expect(cppBasicsTopic.subsetQuestionCount).toBeLessThan(cppBasicsTopic.questionCount)
    expect(cppBasicsTopic.cheatsheet).toHaveLength(6)
    expect(cppBasicsTopic.lastReviewed).toMatch(/^\d{4}-\d{2}-\d{2}$/)

    for (const section of cppBasicsTopic.cheatsheet) {

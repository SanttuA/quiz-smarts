...
import csharpBasicsTopic from '.'

describe('Basic C# topic content', () => {
  it('contains ten valid questions of each supported kind', () => {
    expect(csharpBasicsTopic.questions).toHaveLength(40)
    for (const kind of ['multiple-choice', 'text-blank', 'drag-blank', 'sequence'] as const) {
      expect(csharpBasicsTopic.questions.filter((question) => question.kind === kind)).toHaveLength(
        10,
      )
    }
  })
  ...
  it('has unique namespaced IDs and valid answer references', () => {
    const ids = csharpBasicsTopic.questions.map((question) => question.id)
    expect(new Set(ids).size).toBe(ids.length)

    for (const question of csharpBasicsTopic.questions) {
      expect(question.id).toMatch(/^csharp-basics\./)
      expect(question.topicId).toBe(csharpBasicsTopic.id)
  ...
  it('keeps metadata and reference material in sync', () => {
    expect(csharpBasicsTopic.questionCount).toBe(csharpBasicsTopic.questions.length)
    expect(csharpBasicsTopic.subsetQuestionCount).toBeGreaterThan(0)
    expect(csharpBasicsTopic.subsetQuestionCount).toBeLessThan(csharpBasicsTopic.questionCount)
    expect(csharpBasicsTopic.cheatsheet).toHaveLength(6)
    expect(csharpBasicsTopic.lastReviewed).toMatch(/^\d{4}-\d{2}-\d{2}$/)

    for (const section of csharpBasicsTopic.cheatsheet) {

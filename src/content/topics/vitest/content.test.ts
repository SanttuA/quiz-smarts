...
describe('Vitest topic content', () => {
  it('contains ten valid questions of each supported kind', () => {
    expect(vitestTopic.questions).toHaveLength(40)
    for (const kind of ['multiple-choice', 'text-blank', 'drag-blank', 'sequence'] as const) {
      expect(vitestTopic.questions.filter((question) => question.kind === kind)).toHaveLength(10)
    }
  })

  it('has unique namespaced IDs and valid official answer references', () => {
    const ids = vitestTopic.questions.map((question) => question.id)
    expect(new Set(ids).size).toBe(ids.length)

    for (const question of vitestTopic.questions) {
      expect(question.id).toMatch(/^vitest\./)
      expect(question.topicId).toBe(vitestTopic.id)
      expect(question.explanation.length).toBeGreaterThan(20)
      expect(question.reference.url).toMatch(/^https:\/\/(vitest\.dev|testing-library\.com)\//)
  ...
  it('keeps metadata and reference material in sync', () => {
    expect(vitestTopic.questionCount).toBe(vitestTopic.questions.length)
    expect(vitestTopic.subsetQuestionCount).toBeGreaterThan(0)
    expect(vitestTopic.subsetQuestionCount).toBeLessThan(vitestTopic.questionCount)
    expect(vitestTopic.cheatsheet).toHaveLength(6)
